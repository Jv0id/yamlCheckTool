 ...
 * <pre>
 *     java -cp yamlCheckTool.jar com.fs.Cli -h
 * </pre>
 * or
 * <pre>
 *     java -cp yamlCheckTool.jar com.fs.Cli --help
 * </pre>
 */
public final class Cli {
     ...
     */
    public static final Map<String, Format.OutputFormat> OUTPUT_FORMATS = Collections.unmodifiableMap(
            Stream.of(
                    new AbstractMap.SimpleEntry<>("parsable", Format.OutputFormat.PARSABLE),
                    new AbstractMap.SimpleEntry<>("standard", Format.OutputFormat.STANDARD),
                    new AbstractMap.SimpleEntry<>("colored", Format.OutputFormat.COLORED),
                    new AbstractMap.SimpleEntry<>("github", Format.OutputFormat.GITHUB),
                    new AbstractMap.SimpleEntry<>("auto", Format.OutputFormat.AUTO)
    ...
    private Options prepareOptions() {
        ...
        options.addOption(Option.builder("h").longOpt(ARG_HELP).hasArg(false).argName(ARG_HELP).desc("显示此帮助信息并退出").build());
        options.addOption(Option.builder("v").longOpt(ARG_VERSION).hasArg(false).argName(ARG_VERSION).desc("显示程序的版本号并退出").build());

        og.addOption(Option.builder("c").longOpt("config-file").hasArg().argName(ARG_CONFIG_FILE).desc("自定义配置的路径").build());
        og.addOption(Option.builder("d").longOpt("config-data").hasArg().argName(ARG_CONFIG_DATA).desc("自定义配置（作为YAML源）").build());
        options.addOptionGroup(og);

        options.addOption(Option.builder("f").longOpt(ARG_FORMAT).hasArg().argName(ARG_FORMAT).desc("解析并格式化输出为: " +
                OUTPUT_FORMATS.keySet().stream().map(f -> (DEFAULT_FORMAT.equals(f)) ? ("'" + f + "' (default)") : ("'" + f + "'")).collect(Collectors.joining(", "))).build());
        options.addOption(Option.builder().longOpt(ARG_NO_WARNINGS).hasArg(false).argName(ARG_NO_WARNINGS).desc("仅输出错误级别日志").build());
        options.addOption(Option.builder("s").longOpt(ARG_STRICT).hasArg(false).argName(ARG_STRICT).desc("在警告和错误上返回非零退出代码").build());
    ...
    private CommandLine parseCommandLine(final Options options, final String[] args) {
        ...
        try {
            ...
            if (cmdLine.hasOption(ARG_VERSION)) {
                ...
                err("yamlCheckTool" + " " + props.getProperty("version"));
                System.exit(0);
            }
    ...
    private void showHelpAndExit(Options options, OutputStream output, int exitCode) {
        ...
        if (options == null) {
            ...
            formatter.printHelp(
                    ...
                    width,
                    syntax,
                    "\n一个用于YAML文件的检查工具。yamllint不仅检查语法的有效性, 也可以对于像键重复和美观问题（如行长度、尾随空格、缩进等）这样的情况进行输出.\n\n" +
                            "位置参数：\n" +
                            " FILE_OR_DIR                      要检查的文件或从标准输入读取\n\n" +
                            "可选参数：",
                    options,
