        ...
        while (true) {
            ...
            if (indent != expectedIndent) {
                problems.add(new LintProblem(lineNo, indent + 1,
                        "错误的缩进：预期为 " + expectedIndent + " 但实际为 " + indent));
            }
        }
        ...
        if (firstInLine) {
            ...
            if (!foundIndentation.equals(expected)) {
                problems.add(new LintProblem(token.getStartMark().getLine() + 1, foundIndentation + 1,
                        "错误的缩进：预期为 " + expected + " 但实际为 " + foundIndentation));
            }
        }

        if (token instanceof ScalarToken && (boolean)conf.get(OPTION_CHECK_MULTI_LINE_STRINGS)) {
